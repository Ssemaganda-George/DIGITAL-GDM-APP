...
Original file is located at
    https://colab.research.google.com/drive/1O_D95SkHmKwPGYmDZgCG32xS-gPb9RM7
"""
...
# def evaluate_relevance(expected, generated):
#     expected_embedding = get_embedding(expected)
#     generated_embedding = get_embedding(generated)
#     # Compute cosine similarity
#     similarity = 1 - cosine(expected_embedding, generated_embedding)
#     return similarity

# # Test cases with expected responses
# test_cases = [
#     {"question": "When should induction or cesarean delivery be considered for pregnant women with gestational diabetes and at how many weeks should induction be recommended , and how do macrosomia and glucose management affect this decision", "expected": "For women with well-controlled gestational diabetes (GDM) managed by diet and lifestyle (GDM A1), induction is generally recommended at 41 weeks, but can be discussed at 40 weeks. For women with GDM requiring medication (GDM A2), induction is recommended at 39 weeks. Macrosomia, or large fetal size, can influence the decision, with cesarean delivery being considered if the estimated fetal weight exceeds 4500 g to avoid birth trauma. Regular third-trimester ultrasounds should be done to assess fetal size and guide discussions on the risks and benefits of induction versus cesarean delivery."},
#     {"question": "How should gestational diabetes (GDM) and newly diagnosed type 2 diabetes in pregnancy be managed, and why is postpartum screening and long-term monitoring important?", "expected": "Managing gestational diabetes (GDM) and newly diagnosed type 2 diabetes in pregnancy is crucial to minimize complications for both mother and child. While the best screening, diagnostic tests, and management practices for GDM are still debated, strict guidelines may improve pregnancy outcomes. Postpartum screening and long-term monitoring are essential due to the increased risk of developing type 2 diabetes later in life for women who had GDM. Lifestyle interventions and metformin can significantly reduce this progression."},
# ]
...
# # Evaluate relevance for each test case
...
#         print(f"Error decoding JSON response for question: {question}")
#         generated = ""

#     # Calculate Relevance Score if generated response is available
#     if generated:
#         relevance_score = evaluate_relevance(expected, generated)
#     else:
#         relevance_score = 0
...
# # Relevance Scores
# plt.subplot(1, 2, 1)
# sns.barplot(x=questions, y=relevance_scores, palette="viridis")
# plt.xticks(rotation=90)
# plt.xlabel('Questions')
# plt.ylabel('Relevance Score')
# plt.title('Relevance Scores for Test Cases')
...
sns.barplot(x='Prompt', y='Consistency', data=df_results)
plt.xticks(rotation=90, fontsize=10)
plt.title('Consistency of Responses', fontsize=12)

# Plot Relevance
plt.subplot(3, 4, 2)
sns.barplot(x='Prompt', y='Relevance', data=df_results)
plt.xticks(rotation=90, fontsize=10)
plt.title('Relevance of Responses', fontsize=12)

# Plot Novelty
plt.subplot(3, 4, 3)
sns.barplot(x='Prompt', y='Novelty', data=df_results)
plt.xticks(rotation=90, fontsize=10)
plt.title('Novelty of Responses', fontsize=12)

# Plot Hate Speech
plt.subplot(3, 4, 4)
sns.countplot(x='Hate Speech', data=df_results)
plt.title('Hate Speech Detection', fontsize=12)

# Plot Bias
plt.subplot(3, 4, 5)
sns.histplot(df_results['Bias'], bins=20)
plt.title('Bias Detection', fontsize=12)
...
for metric in ['ROUGE1', 'ROUGE2', 'ROUGEL']:
    plt.plot(df_results.index, df_results[metric], label=metric)
plt.legend()
plt.title('ROUGE Scores', fontsize=12)

# Plot Time Taken
plt.subplot(3, 4, 7)
sns.barplot(x='Prompt', y='Time (s)', data=df_results)
plt.xticks(rotation=90, fontsize=10)
plt.title('Time Taken for Responses', fontsize=12)

# Plot Memory Used
plt.subplot(3, 4, 8)
sns.barplot(x='Prompt', y='Memory (MB)', data=df_results)
plt.xticks(rotation=90, fontsize=10)
plt.title('Memory Used', fontsize=12)
...
# Print Summary
print("\nSummary of Results:")
print(df_results[['Prompt', 'Consistency', 'Relevance', 'Novelty', 'Hate Speech', 'Bias', 'Time (s)', 'Memory (MB)', 'CPU Usage (%)', 'ROUGE1', 'ROUGE2', 'ROUGEL']].to_string(index=False))
...
def save_plot(data, plot_type, x_col, y_col, title, xlabel, ylabel, filename):
    plt.figure(figsize=(10, 6))
    if plot_type == 'bar':
...
plt.ylabel('Score', fontsize=12)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'rouge_scores_plot.png'))
plt.close()
...
save_plot(df_results, 'bar', 'Prompt', 'Memory (MB)', 'Memory Used', 'Prompt', 'Memory (MB)', 'memory_used_plot.png')
